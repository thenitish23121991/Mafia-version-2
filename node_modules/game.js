...
if(role1 == 'mafia'){
var user_json = {'name':user,'role':role1,'status':'alive','player_type':'host'};
var god_json = {'name':'god','role':'god','status':'alive','player_type':'god'};
game_users.push(user_json);
...
invite_user_to_game : function(game,user,medium,sender,callback){
	
if(medium == 'message'){
var result = user_requests.insert({game:game,user:user,medium:medium,sent_by:sender});
result.on('complete',function(err,docs){
callback(docs);	
});	
}	
},

accept_invitation_to_game : function(game,user,callback){
var result = invites.update({game:game,user:user},{$set:{accepted:true}});	
...
add_user_comment : function(user,game,comment){
	
var result = comments.insert({game:game,user:user,comment:comment});
